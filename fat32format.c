...
typedef struct 
    ...
    int ReservedSectCount; // any number >= 8 or -1 if align files to cluster (for flash drives) 
    }
format_params;
...
void zero_sectors ( HANDLE hDevice, DWORD Sector, DWORD BytesPerSect, DWORD NumSects, DISK_GEOMETRY* pdgDrive  )
{
    ...
    printf ( "Wrote %" PRId64 " bytes in %.2f seconds, %.2f Megabytes/sec\n", qBytesTotal, fTime, fBytesTotal/(fTime*1024.0*1024.0) );
...
int format_volume ( char vol, format_params* params )
{
    ...
    if(params->ReservedSectCount == -1) {
      int align = 32;
      FatSize = get_fat_size_sectors ( pFAT32BootSect->dTotSec32, 2 * align, pFAT32BootSect->bSecPerClus, pFAT32BootSect->bNumFATs, BytesPerSect ); ;
      ReservedSectCount = align + align - (align + NumFATs*FatSize) % align;
    }
    else {
      ReservedSectCount = params->ReservedSectCount;
      FatSize = get_fat_size_sectors ( pFAT32BootSect->dTotSec32, ReservedSectCount, pFAT32BootSect->bSecPerClus, pFAT32BootSect->bNumFATs, BytesPerSect ); ;
    }
    pFAT32BootSect->wRsvdSecCnt = (WORD) ReservedSectCount;
...
void usage( void )
{
        ...
        printf ( "-rN extra option to set count of reserved sector ( >= 32) or files will automatically aligned to 32 sectors for modern flash drives\n" );
...
int main(int argc, char* argv[])
{
    ...
    while ( (strlen(argv[i])>=2) && ((argv[i][0] == '-')||(argv[i][0] == '/')) ) {
      switch ( argv[i][1] ) {
        ...
        case('r'): {
          if ( strlen(argv[i]) >=3 ) {
            p.ReservedSectCount = atol( &argv[i][2] );
            if (p.ReservedSectCount < 8               
            ) {
              printf ( "Ignoring bad resered sector count %d\n", p.sectors_per_cluster );
              p.ReservedSectCount = -1;
            }
          }
          else {
